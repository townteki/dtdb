...
class EmailCommand extends ContainerAwareCommand
{
    ...
    /**
     * @param $user \Dtdb\UserBundle\Entity\User
     */
    protected function sendArchive($em, $user, $path)
    {
        ...
        // Give it a body
        $message->setBody($this->getContainer()->get('templating')->render('DtdbBuilderBundle:Emails:deck_archive.txt.twig', array(
        	'username' => $user->getUsername()
        )));
        
        // And optionally an alternative body
        $message->addPart($this->getContainer()->get('templating')->render('DtdbBuilderBundle:Emails:deck_archive.html.twig', array(
        	'username' => $user->getUsername()
        )), 'text/html');
        
        $attachment = \Swift_Attachment::fromPath($path, 'application/zip')->setFilename('dtdb.zip');
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if($user_id) {
            $users[] = $em->getRepository('DtdbUserBundle:User')->find($user_id);
        } else {
            $users = $em->getRepository('DtdbUserBundle:User')->findBy(array('locked' => FALSE));
        }
        $nb=3;
