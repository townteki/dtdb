...
namespace Dtdb\CardsBundle\Services;
...
class CardsData
{
	...
	{
		$list_packs = $this->doctrine->getRepository('DtdbCardsBundle:Pack')->findBy(array(), array("released" => "ASC", "number" => "ASC"));
		$packs = array();
		$sreal=0; $smax = 0;
				...
				case 'e': // expansion
					...
					foreach($condition as $arg) {
						switch($operator) {
							...
							case '<':
							    if(!isset($qb2)) {
							        $qb2 = $this->doctrine->getRepository('DtdbCardsBundle:Pack')->createQueryBuilder('p2');
							        $or[] = $qb->expr()->lt('p.released', '(' . $qb2->select('p2.released')->where("p2.code = ?$i")->getDql() . ')');
							    }
							    break;
							case '>':
							    if(!isset($qb3)) {
							        $qb3 = $this->doctrine->getRepository('DtdbCardsBundle:Pack')->createQueryBuilder('p3');
