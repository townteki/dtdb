...
class CardsData
{
    ...
    public function __construct(
        ...
        $this->router = $router;
        $this->imagesDir = $kernelProjectDirectory . '/public/images/cards';
    }
    ...
    {
        $list_packs = $this->doctrine
            ->getRepository(Pack::class)
            ->findBy(array(), array("released" => "ASC", "number" => "ASC"));
        $packs = array();
        $sreal = 0;
        $smax = 0;
    ...
    {
        $list_cycles = $this->doctrine->getRepository(Cycle::class)->findBy(array(), array("number" => "ASC"));
        $packs = array();
    ...
    public function get_search_rows($conditions, $sortorder, $forceempty = false)
    {
        ...
        $qb = $this->doctrine->getRepository(Card::class)->createQueryBuilder('c');
                ...
                case 'e': // expansion
                    ...
                    foreach ($condition as $arg) {
                        switch ($operator) {
                            ...
                            case '<':
                                if (!isset($qb2)) {
                                    $qb2 = $this->doctrine->getRepository(Pack::class)->createQueryBuilder('p2');
                                    $or[] = $qb->expr()->lt(
                                        'p.released',
                                        '(' . $qb2->select('p2.released')->where("p2.code = ?$i")->getDql() . ')'
                                    );
                                }
                                break;
                            case '>':
                                if (!isset($qb3)) {
                                    $qb3 = $this->doctrine->getRepository(Pack::class)->createQueryBuilder('p3');
                                    $or[] = $qb->expr()->gt(
                                        'p.released',
                                        '(' . $qb3->select('p3.released')->where("p3.code = ?$i")->getDql() . ')'
                                    );
                                }
                                break;
                        }
                    ...
                    foreach ($condition as $arg) {
                        switch ($operator) {
                            case '<':
                                ...
                                break;
                            case '>':
                                $or[] = "(p.released > ?$i or p.released IS NULL)";
                                break;
                        }
                        if ($arg == "now") {
        ...
        switch ($sortorder) {
            ...
            case 'cost':
                ...
                break;
            case 'rank':
                $qb->orderBy('c.rank')->addOrderBy('c.type')->addOrderBy('c.gang');
                break;
        }
        ...
        for ($i = 0; $i < count($rows); $i++) {
            while (
                isset($rows[$i + 1]) && $rows[$i]->getTitle() == $rows[$i + 1]->getTitle() && $rows[$i]->getPack(
                )->getCode() == "alt"
            ) {
                $rows[$i] = $rows[$i + 1];
    ...
    public function getCardAlternatives($card)
    {
        ...
        $alternatives = array();
        foreach ($rows as $alt) {
            if ($alt->getPack()->getId() == $card->getPack()->getId()) {
                continue;
            }
            $alternatives[] = array(
    ...
    public function getCardInfo($card, $api = false)
    {
        ...
        $cardinfo['url'] = $this->router->generate(
            'cards_zoom',
            array('card_code' => $card->getCode(), '_locale' => $locale),
            true
        );

        $cardinfo['imagesrc'] = "";

        if ($locale != 'en' && file_exists($this->imagesDir . '/' . $locale . '/' . $card->getCode() . '.jpg')) {
            $cardinfo['imagesrc'] = "/images/cards/$locale/" . $card->getCode() . '.jpg';
        } elseif (file_exists($this->imagesDir . '/en/' . $card->getCode() . '.jpg')) {
            $cardinfo['imagesrc'] = "/images/cards/en/" . $card->getCode() . '.jpg';
        }
    ...
    public function validateConditions(&$conditions)
    {
        ...
        foreach ($conditions as $i => $l) {
            ...
            if ($l[0] == 'g') {
                ...
                if (!in_array($conditions[$i][2], $gangs)) {
                    unset($conditions[$i]);
                }
            }
        }
    ...
    {
        $reviews = $this->doctrine
            ->getRepository(Review::class)
            ->findBy(array('card' => $card), array('nbvotes' => 'DESC'));
