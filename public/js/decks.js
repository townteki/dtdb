...
function do_action_deck(event) {
    ...
    switch (action_id) {
        case 'btn-view':
            location.href = Routing.generate('deck_view', {deck_id: SelectedDeck.id});
            break;
        case 'btn-edit':
            location.href = Routing.generate('deck_edit', {deck_id: SelectedDeck.id});
            break;
        case 'btn-publish':
            confirm_publish();
            break;
        case 'btn-duplicate':
            location.href = Routing.generate('deck_duplicate', {deck_id: SelectedDeck.id});
            break;
        case 'btn-delete':
            confirm_delete();
            break;
        case 'btn-download-text':
            location.href = Routing.generate('deck_export_text', {deck_id: SelectedDeck.id});
            break;
        case 'btn-download-octgn':
            location.href = Routing.generate('deck_export_octgn', {deck_id: SelectedDeck.id});
            break;
...
function do_action_selection(event) {
    ...
    switch (action_id) {
        ...
        case 'btn-delete-selected':
            confirm_delete_all(ids);
            break;
        case 'btn-download-text':
            download_text_selection(ids);
            break;
        case 'btn-download-octgn':
            download_octgn_selection(ids);
            break;
    }
}
...
function do_action_sort(event) {
    ...
    switch (action_id) {
        case 'btn-sort-update':
            sort_list('dateupdate');
            break;
        case 'btn-sort-creation':
            sort_list('datecreation');
            break;
        case 'btn-sort-outfit':
            sort_list('outfit_title,name');
            break;
        case 'btn-sort-gang':
            sort_list('gang_code,name');
            break;
        case 'btn-sort-lastpack':
            sort_list('cycle_id,pack_number');
            break;
        case 'btn-sort-name':
            sort_list('name');
            break;
...
function sort_list(type) {
    ...
    var sort = type.split(/,/).map(function (t) {
        return t + ' ' + (order > 0 ? 'desc' : 'asec')
    }).join(',');
